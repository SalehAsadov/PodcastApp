...
// MARK: - Selectors
extension EpisodeViewController{
    @objc private func handleFavoritButton(){
        isFavorite ? deleteCoreData() : addCoreData()
    }
}
// MARK: - Helpers
extension EpisodeViewController{
    ...
    private func setupNavBarItem(){     
        let navRightItem = UIBarButtonItem(image: UIImage(systemName: isFavorite ? "heart.fill" : "heart")?.withTintColor(.systemPink, renderingMode: .alwaysOriginal), style: .done, target: self, action: #selector(handleFavoritButton))
